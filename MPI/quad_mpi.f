...
c
     ...
     &    MPI_COMM_WORLD, status, error_flag )
        ...
        write ( *, '(a,i8,a,g14.6, x,a,2(x,g14.6),i8)' ) 
...
c
c      call MPI_Reduce ( my_total, total, 1, MPI_DOUBLE_PRECISION, 
c     &  MPI_SUM, master, MPI_COMM_WORLD, error_flag )
      call MPI_AllReduce ( my_total, total, 1, MPI_DOUBLE_PRECISION, 
     &  MPI_SUM, MPI_COMM_WORLD, error_flag )
c
c
c  Compute the weighted estimate.
c
c      if ( my_id .eq. master ) then
        error = abs ( total - exact )
        wtime = MPI_Wtime ( ) - wtime
        write (*,'(a,I8)')'my ID = ', my_id
        write ( *, '(a)' ) ' '
